 ...
 * @property {string?} chatName - chat title
...
async function handleRu(message) {
    bot.sendMessage(message.chat.id, "москаль в стойло", {
        reply_to_message_id: message.message_id,
    });
}
...
function buildMessage(tgMessage) {
    return {
        ...
        chatName: tgMessage.chat.title ?? null,
...
function punish(message, report, ban) {
    ...
    bot.deleteMessage(message.chatId, message.id);
    chat.deletedMessages++;

    if (ban === true) {
        bot.banChatMember(message.chatId, message.senderId);
        chat.bannedSpammers++;
    }
...
setInterval(
    async () => {
        ...
        for (const chat of chats) {
            ...
            const premiumDur =
                new Date().getTime() - chat.premiumedOn.getTime();
